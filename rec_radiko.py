...
if __name__ == '__main__':
    ...
    cmd = cmd + ' -acodec libmp3lame -ab 128k {}'
    cmd = cmd.format( path.format( outdir, prefix, date ) )
    # Exec ffmpeg
    subprocess.call( cmd.strip().split(" ")  ) 
    ...
    tags = EasyID3(path)
    tags['album'] = channel
    if program.title[index] is not None:
        tags['title'] = program.title[index].text
    if program.pfm[index] is not None:
        tags['artist'] = program.pfm[index].text

    tags.save()

    if program.img[index] is not None:
